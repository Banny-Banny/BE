...
- 옵션 선택: 열람 시점(1주/1달/1년/직접입력), 인원수(최대 10명), 사진(장당 500원, 인원당 최대 5장 → 총 사진 ≤ 인원수*5), 추가 옵션(배경음악 인당 1000원, 동영상 인당 2000원) — 음악/동영상은 구매자만 업로드 가능.
...
4. 사진 개수를 선택한다(장당 500원, 인원당 최대 5장 → 총 사진 ≤ 인원수*5).
5. 추가 옵션으로 배경음악(인당 1000원), 동영상(인당 2000원)을 선택할 수 있다(구매자만 업로드 가능).
...
5. **사진**:
   - 사진 단가 500원.
   - 제약: `photo_count ≤ headcount * 5`. 초과 시 400.
6. **추가 옵션**:
   - 배경음악: 인당 1000원. 구매자만 업로드 가능.
   - 동영상: 인당 2000원. 구매자만 업로드 가능.
7. **기본 상품가**: 1주 옵션 1000원을 baseline으로 적용(1달/1년/CUSTOM도 동일 기본가; 정책 확장 시 업데이트).
8. **총액 계산**:
   - `total_amount = 1000 + (photo_count * 500) + (add_music ? headcount * 1000 : 0) + (add_video ? headcount * 2000 : 0)`.
9. **저장**: order 테이블(또는 orders 엔티티)에 옵션과 금액을 저장하고 상태는 “PENDING_PAYMENT” 등으로 설정.
10. **응답**: 201에 주문 정보 반환 `{ order_id, total_amount, time_option, custom_open_at?, headcount, photo_count, add_music, add_video, status }`.
